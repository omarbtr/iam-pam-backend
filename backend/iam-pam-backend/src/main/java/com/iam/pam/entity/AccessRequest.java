...
public class AccessRequest {
    ...
    // Horodatage du premier accès réel (SSH/RDP/Web/DB) — le timer démarre ici
    @Column(name = "first_access_at")
    private LocalDateTime firstAccessAt;

    // Chemin du fichier d'enregistrement RDP (généré par guacd)
    ...
    public boolean isActive() {
        if (status != RequestStatus.APPROVED) return false;
        // Si firstAccessAt est défini et durationHours aussi, l'expiry est calculé dynamiquement
        if (firstAccessAt != null && durationHours != null) {
            return firstAccessAt.plusHours(durationHours).isAfter(LocalDateTime.now());
        }
        // Pas encore accédé = toujours actif (le timer n'a pas démarré)
        if (firstAccessAt == null && durationHours != null) return true;
        return expiresAt == null || expiresAt.isAfter(LocalDateTime.now());
    }

    public LocalDateTime computeExpiresAt() {
        if (firstAccessAt != null && durationHours != null) {
            return firstAccessAt.plusHours(durationHours);
        }
        return expiresAt;
    }
