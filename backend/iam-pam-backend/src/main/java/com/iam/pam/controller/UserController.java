...
@Tag(name = "User Management", description = "Gestion des utilisateurs du tenant via AD/LDAP (tenant-admin)")
@RestController
...
public class UserController {
     ...
     */
    @Operation(summary = "Rechercher des utilisateurs dans l'annuaire AD/LDAP (tenant-admin)")
     ...
     */
    @Operation(summary = "Lister les utilisateurs de l'annuaire paginés (tenant-admin)")
    ...
    public ResponseEntity<ApiResponse<List<UserDTO.DirectoryUser>>> listDirectory(
        ...
        int safeMax = Math.min(Math.max(1, max), 100);
        return ResponseEntity.ok(
                ApiResponse.success(keycloakAdminService.listDirectoryUsers(safeFirst, safeMax),
     ...
     */
    @Operation(summary = "Importer un utilisateur depuis l'annuaire AD dans le tenant (tenant-admin)")
    @PostMapping("/import")
    @PreAuthorize("hasRole('tenant-admin')")
     ...
     */
    @Operation(summary = "Retirer un utilisateur du tenant (tenant-admin)")
     ...
     */
    @Operation(summary = "Lister les utilisateurs du tenant actuel (tenant-admin)")
    @GetMapping
    @PreAuthorize("hasRole('tenant-admin')")
     ...
     */
    @Operation(summary = "Vérifier si le tenant peut ajouter des utilisateurs (tenant-admin)")
    @GetMapping("/can-add")
    @PreAuthorize("hasRole('tenant-admin')")
